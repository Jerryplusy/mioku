...
export const run = async (ctx: IcqqCliContext): Promise<IcqqCliConfig> => {
  ...
  let addMore = true;
  while (addMore) {
    const uinRaw = await input("QQ 号 (uin)", {
      required: true,
    ...
    const signApiAddr = await input(
      "qsign /sign 地址 (可空，如 http://127.0.0.1:8080/sign?key=xxx)",
      {
        placeholder: "可空",
      },
    );
    const platformRaw = (await consola.prompt("登录设备类型", {
      type: "select",
      cancel: "reject",
      initial: "aphone",
      options: [
        { label: "aphone（安卓手机，默认）", value: "aphone" },
        { label: "apad（安卓平板）", value: "apad" },
        { label: "awatch（安卓手表）", value: "awatch" },
        { label: "ipad", value: "ipad" },
        { label: "imac（macOS）", value: "imac" },
      ],
    })) as string | { value?: string } | null;
    const platform =
      typeof platformRaw === "string"
        ? platformRaw
        : (platformRaw?.value ?? "aphone");
    const ignoreSelf = await confirm("忽略自己账号发送的消息？", {
      initial: true,
    });
