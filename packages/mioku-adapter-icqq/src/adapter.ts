...
import { toIcqqMessage } from './message/convert'
import type { Adapter, AdapterContext, AdapterFactoryOptions, Bot } from 'mioku'
  ...
  const registerBotCapabilities = (ctx: AdapterContext, currentBot: IcqqBot): Array<() => void> => {
    ...
    return [
      ...
      ctx.registerCapability(memberPoke, target, async (req) => {
        await currentBot.sendApi('send_group_poke', {
      ...
      ctx.registerCapability(profileSet, target, async (req) => {
        ...
        if (typeof req.personal_note === 'string') await c.setSignature(req.personal_note)
        if (typeof req.sex === 'number') await c.setGender(req.sex as 0 | 1 | 2)
      }),
      ...
      ctx.registerCapability(botStatus, target, async () => {
        try {
          ...
          return { online: Boolean(info && info.status === 11), app_name: 'icqq' }
        } catch {
  ...
  return {
    ...
    async start(next) {
      ...
      bind('system.login.qrcode', (event) => {
        ...
        const file = path.join(os.tmpdir(), `mioku-icqq-qrcode-${index + 1}.png`)
        try {
