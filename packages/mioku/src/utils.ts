...
export const createCmd = (
  ...
  if (prefix) {
    if (cmd !== prefix)
      return { cmd: undefined, params: [], options: cmdOptions };
...
export const createStore = async <T extends object = object>(
  ...
  const dirname =
    __dirname ||
    meta?.dirname ||
    (meta?.url ? path.dirname(fileURLToPath(meta.url)) : "");
  if (!dirname)
    throw new Error(
      "createStore: options.__dirname or options.meta must be provided",
    );
...
export const md5 = (
  text: BinaryLike,
  encoding: BinaryToTextEncoding | "buffer" = "hex",
): string | Buffer => {
...
export const randomItem = <T>(
  ...
  if (!Array.isArray(array) || !array.length)
    throw new Error("randomItem: 参数必须是数组，且不能为空");
...
export const randomItems = <T>(
  ...
  if (!Array.isArray(array) || !array.length)
    throw new Error("randomItems: 参数必须是数组，且不能为空");
  if (count < 0) throw new Error("randomItems: count 必须为非负整数");
  if (count === 0) return [];
  if (count > array.length)
    throw new Error(
      `randomItems: 要选择的数量 (${count}) 超过了数组长度 (${array.length})`,
    );
  ...
  for (let i = 0; i < count; i++) {
    ...
    [indices[randomIdx], indices[lastIdx]] = [
      indices[lastIdx],
      indices[randomIdx],
    ];
  }
  ...
  return hasString
    ? items.sort((p, n) =>
        isString(p) && isString(n) ? p.localeCompare(n) : 0,
      )
    : items;
};
...
export const noNullish = <T>(val: T | null | undefined): val is T =>
  val !== null && val !== undefined;

export const isDefined = <T = unknown>(val?: T): val is T =>
  typeof val !== "undefined";

export const isFunction = <T extends AnyFunc = AnyFunc>(
  val: unknown,
...
export const localNum = (num: number, locale = "zh-CN"): string =>
  num.toLocaleString(locale);

export const base64Encode = (str: string | number | Buffer): string =>
  Buffer.from(str.toString()).toString("base64");

export const base64Decode = (
  str: string,
  type: "buffer" | BufferEncoding = "utf8",
): string | Buffer => {
  if (type === "buffer") return Buffer.from(str, "base64");
  return Buffer.from(str, "base64").toString(type);
};

export const qs = (obj: Record<number | string, unknown>): string =>
...
export type MatchHandler<T extends HasMessage> = (
  ...
  for (const [key, value] of Object.entries(pattern)) {
    ...
    const result =
      typeof value === "function"
        ? await value(matches as RegExpMatchArray, event)
        : value;
    if (result) {
      const reply = (
        event as unknown as {
...
export const find = <
  T extends Message["length"] extends 0 ? never : Message[number],
>(
  source: HasMessage | Message,
  type: string,
): T | undefined => {
...
const ADAPTER_DISPLAY_NAMES: Readonly<Record<string, string>> = {
  stdin: "标准输入",
  onebotv11: "OneBot v11",
  icqq: "ICQQ",
};

export const displayAdapterId = (
  id: string | number | null | undefined,
): string => {
  if (id == null) return "";
  return ADAPTER_DISPLAY_NAMES[String(id)] ?? String(id);
};

export { type ConsolaInstance };

const getMessage = (source: HasMessage | Message): Message => {
  if (Array.isArray(source) && typeof (source as Message).text === "function")
