...
import { validatePluginPackageConfig } from './manifest'

import type { PluginPackageConfig } from '../types'
...
export interface PluginCandidate {
  ...
  readonly entry?: string
  readonly description?: string
  readonly config: PluginPackageConfig
}

const ADAPTER_PACKAGE_PREFIX = 'mioku-adapter-'
const PLUGIN_PACKAGE_PREFIX = 'mioku-plugin-'
...
export const discoverPluginCandidates = (cwd: string, appPackageJson: PackageJson): readonly PluginCandidate[] => {
  ...
  for (const dep of deps) {
    ...
    candidates.push({
      ...
      apiVersion: 1,
      description: pkg.description,
      config: validatePluginPackageConfig(pkg.mioku, pluginName),
    })
  }
  return candidates
}
