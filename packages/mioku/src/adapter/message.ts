...
const fileData = (file: string | Buffer, options: { local?: boolean } = {}): Record<string, unknown> => {
  if (Buffer.isBuffer(file)) return { file: `base64://${file.toString('base64')}` }
  if (options.local) return { file: `file:///${file.replace(/^\s*(file:\/\/\/)+/, '')}` }
  return { file }
}
...
export const segment = {
  ...
  file(file: string | Buffer, options: { local?: boolean; name?: string } = {}): MessageSegment {
  ...
  node(options: { user_id: string; nickname: string; content: MessageInput }): MessageSegment {
    return new MessageSegmentImpl('node', {
      user_id: options.user_id,
      nickname: options.nickname,
      content: Array.isArray(options.content) ? Array.from(options.content) : options.content,
    })
  },
